...
<body>
	...
	<div class="detBuku">
		...
		<div class="detailBuku">
			<div class="coverBuku">
				<img class="sorotan" src="{{ $book->thumb }}">
			</div>
			<div class="deskripsi">
				<h1>{{ $book->judul }}</h1>
				<h3>Penulis : {{ $book->penulis }}</h3>
				<h3>Waktu Terbit : {{ $book->waktu_terbit }}</h3>
				<h3>Jumlah Halaman : {{ $book->jumlah_halaman }} Halaman</h3>
				<p>{{ $book->deskripsi }}</p>

				<p class="link" onclick="return document.location.href = 'http://vinlibrary.test/read/{{ $book->slug }}/view';">Baca</p>
			</div>
		</div>
